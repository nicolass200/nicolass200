# 👋 Olá, eu sou Nicolas Carvalho

## Construindo soluções através da tecnologia.

[![Instagram](https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge\&logo=instagram\&logoColor=white)](https://instagram.com/nicolas.rodriigues)
[![E-mail](https://img.shields.io/badge/E--mail-0078D4?style=for-the-badge\&logo=microsoft-outlook\&logoColor=white)](mailto:nicolas.carvalho@somosicev.com)

---

## ✨ Sobre mim

🎓 Estudante de Engenharia de Software no ICEV - Instituto de Ensino Superior (5º período)

💻 Atualmente desenvolvendo projetos Full Stack

🚀 Apaixonado por tecnologia, boas práticas e resolução de problemas

📚 Estudando:

* Análise de Algoritmos
* Teste e Qualidade de Software
* Redes e Sistemas Distribuídos
* Desenvolvimento Mobile

🔧 Tenho interesse em:

* APIs REST
* Sistemas Web
* Aplicativos Mobile
* Automação

🎯 Focado em criar soluções eficientes, escaláveis e que gerem impacto real.

---

## ✨ Projetos em Destaque

### 💰 FinTrack

Sistema de controle financeiro pessoal com:

* Dashboard
* Gestão de receitas e despesas
* Categorias
* Orçamentos e alertas
* Relatórios financeiros

**Tecnologias:**
React • TypeScript • Node.js • Express • PostgreSQL

---

### 🧪 QA Integração

Projeto voltado para automação, integração de sistemas e qualidade de software.

**Tecnologias:**
Node.js • APIs REST • Testes Automatizados • Integrações

---

## ✨ Tech Stack

### Backend

![NodeJS](https://skillicons.dev/icons?i=nodejs,express,nestjs,python)

### Frontend

![Frontend](https://skillicons.dev/icons?i=react,ts,js,html,css)

### Mobile

![Mobile](https://skillicons.dev/icons?i=flutter,dart)

### Banco de Dados

![Database](https://skillicons.dev/icons?i=postgres,mysql,mongodb)

### Ferramentas

![Tools](https://skillicons.dev/icons?i=git,github,postman,vscode)

---


## 🐍 Contribuições

<picture>
  <source
    media="(prefers-color-scheme: dark)"
    srcset="https://raw.githubusercontent.com/nicolass200/nicolass200/output/github-contribution-grid-snake-dark.svg"
  />
  <source
    media="(prefers-color-scheme: light)"
    srcset="https://raw.githubusercontent.com/nicolass200/nicolass200/output/github-contribution-grid-snake.svg"
  />
  <img
    alt="github contribution grid snake animation"
    src="https://raw.githubusercontent.com/nicolass200/nicolass200/output/github-contribution-grid-snake.svg"
  />
</picture>

## ✨ Objetivos

📈 Evoluir como Desenvolvedor Full Stack

🏗️ Aprimorar conhecimentos em Arquitetura de Software

🚀 Contribuir para projetos que gerem impacto real

---

## ✨ Curiosidades

☕ Café + Código = Combinação perfeita.

🎵 Gosto de ouvir música enquanto programo

📚 Estou sempre estudando algo novo

💡 Acredito que tecnologia é uma ferramenta para transformar ideias em soluções
